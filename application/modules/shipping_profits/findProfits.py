...
class Filewriter:
    def __init__(self, jsonstring, filepath, debug=False):
        '''
        To output a .csv file. Use csvmaker()

        # TODO: make another function to export the data to the webapp (as matrix?)

        jsonstring: .json file or output of json.dumps()
        filepath: path of generated .csv

        csv "cell" values are added as class attribute
        dynamically by _get_row()'''
        '''
        # Detect if jsonstring is a file or a json string
        # Throws ValueError if json isn't valid
        if exists(jsonstring):
            with open(jsonstring, "r") as f:
                self.jsonstring = json.load(f)
        else:
            self.jsonstring = json.dumps(jsonstring)
        '''
    ...
    def csvmaker(self):
        '''Call this func w/o argument to make the csv'''
